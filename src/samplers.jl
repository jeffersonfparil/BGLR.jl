...
function rinvGauss(nu::Float64, lambda::Float64)
    ...
    r1 =
        nu / (2 * lambda) *
        (2 * lambda + nu * y2 - sqrt(4 * lambda * nu * y2 + nu * nu * y2 * y2))
    r2 = nu * nu / r1
    if (u < nu / (nu + r1))
        return (r1)
    else
...
"""
function metropLambda(
    tau2::Array{Float64,1},
    lambda::Float64,
    shape1::Float64,
    shape2::Float64,
    max::Float64,
    ...
    logP_old =
        sum(logpdf(Exponential(2.0 / lambda2), tau2)) +
        logpdf(Beta(shape1, shape2), lambda / max) -
        logpdf(Gamma(rate, 1 / shape), 2.0 / lambda2)

    logP_new =
        sum(logpdf(Exponential(2.0 / l2_new), tau2)) +
        logpdf(Beta(shape1, shape2), l_new / max) -
        logpdf(Gamma(rate, 1.0 / shape), 2.0 / l2_new)
